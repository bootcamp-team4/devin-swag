...
describe("save", () => {
  it("stamps updatedAt rather than trusting the caller", async () => {
    const store = createLocalDesignStore(backing);
    const saved = await store.save(design({ updatedAt: "1999-01-01T00:00:00.000Z" }));
    ...
    expect((await store.get(saved.id))?.updatedAt).toBe(saved.updatedAt);
  });

  it("upserts by id instead of appending a duplicate", async () => {
    const store = createLocalDesignStore(backing);
    const first = await store.save(design({ name: "Original" }));
    const second = await store.save({ ...first, name: "Renamed" });
    expect(second.id).toBe(first.id);
    expect(await store.list()).toHaveLength(1);
    expect((await store.list())[0].name).toBe("Renamed");
  });

  it("writes a versioned payload under one namespaced key", async () => {
    const store = createLocalDesignStore(backing);
  ...
  it("surfaces a full backing store as a catchable StorageQuotaError", async () => {
    const store = createLocalDesignStore(backing);
    backing.full = true;
    await expect(store.save(design())).rejects.toThrow(StorageQuotaError);
    backing.full = false;
    expect(await store.list()).toEqual([]);
  });

  it("rethrows failures that are not about quota", async () => {
    const store = createLocalDesignStore(backing);
    backing.setItem = () => {
      throw new TypeError("nope");
    };
    await expect(store.save(design())).rejects.toThrow(TypeError);
  });
});

describe("list", () => {
  it("returns designs newest updatedAt first", async () => {
    seed(backing, [
    ...
    const store = createLocalDesignStore(backing);
    expect((await store.list()).map((entry) => entry.id)).toEqual(["c", "b", "a"]);
  });

  it("is empty when nothing has ever been saved", async () => {
    expect(await createLocalDesignStore(backing).list()).toEqual([]);
  });

  it("drops one corrupt entry rather than losing the gallery", async () => {
    seed(backing, [design({ id: "good" }), { id: "bad", garment: "tarpaulin" }, null, 42]);
    const store = createLocalDesignStore(backing);
    expect((await store.list()).map((entry) => entry.id)).toEqual(["good"]);
  });

  it("tolerates a garbage blob under the key", async () => {
    backing.map.set(DESIGNS_KEY, "{not json at all");
    expect(await createLocalDesignStore(backing).list()).toEqual([]);
  });

  it("tolerates an unknown payload version", async () => {
    seed(backing, [design({ id: "future" })], STORAGE_VERSION + 99);
    const store = createLocalDesignStore(backing);
    expect(await store.list()).toEqual([]);
    // And a later save still works, rather than the store staying wedged.
    expect((await store.save(design({ id: "now" }))).id).toBe("now");
    expect((await store.list()).map((entry) => entry.id)).toEqual(["now"]);
  });

  it("tolerates a payload whose designs field is not an array", async () => {
...
describe("get and remove", () => {
  it("gets by id and returns null for anything unknown", async () => {
    const store = createLocalDesignStore(backing);
    const saved = await store.save(design({ name: "Keeper" }));
    expect((await store.get(saved.id))?.name).toBe("Keeper");
    expect(await store.get("missing")).toBeNull();
  });

  it("removes only the requested design", async () => {
    const store = createLocalDesignStore(backing);
    const first = await store.save(design());
    const second = await store.save(design());
    await store.remove(first.id);
    expect(await store.get(first.id)).toBeNull();
    expect((await store.list()).map((entry) => entry.id)).toEqual([second.id]);
  });
  ...
  it("ignores removing something that is not there", async () => {
    ...
    expect((await store.list()).map((entry) => entry.id)).toEqual([saved.id]);
  });
});
...
describe("draft", () => {
  ...
  it("keeps the draft out of the named designs", async () => {
    const store = createLocalDesignStore(backing);
...
describe("subscribe", () => {
  it("notifies on writes and stops after unsubscribing", async () => {
    const store = createLocalDesignStore(backing);
  ...
  it("does not notify for a no-op remove", async () => {
    const store = createLocalDesignStore(backing);
    const listener = vi.fn();
